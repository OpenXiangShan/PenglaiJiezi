    ...
    private:
        ...
        void printStack(const char *when) {
            ...
            for (int i = 0; i < numEntries; i++) {
                DPRINTFR(FTBRAS, "entry [%d], retAddr %#lx, ctr %d",
                     i, stack[i].data.retAddr, stack[i].data.ctr);
                if (ssp == i) {
                    DPRINTFR(FTBRAS, " <-- SSP");
                }
                if (nsp == i) {
            ...
            for (int i = 0; i < numInflightEntries; i++) {
                DPRINTFR(FTBRAS, "entry [%d] retAddr %#lx, ctr %u nos %d",
                i, inflightStack[i].data.retAddr, inflightStack[i].data.ctr,
                inflightStack[i].nos);
                if (TOSW == i) {
