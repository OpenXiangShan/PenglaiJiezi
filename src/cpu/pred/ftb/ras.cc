...
RAS::setTrace()
{
    if (enableDB) {
        ...
        // record every modification to the non-spec-stack,
        ...
        nonSpecRasTrace = _db->addAndGetTrace("NONSPECRASTRACE",
                    nonspec_fields_vec);
...
RAS::specUpdateHist(const boost::dynamic_bitset<> &history, FullFTBPrediction &pred)
{
    ...
    if (takenSlot.isCall) {
        ...
        if (enableDB) {
            SpecRASTrace rec(When::SPECULATIVE,
                RAS_OP::PUSH, pred.bbStart, takenSlot.pc,
    ...
    if (takenSlot.isReturn) {
        if (enableDB) {
            SpecRASTrace rec(When::SPECULATIVE,
                RAS_OP::POP, pred.bbStart, takenSlot.pc,
...
RAS::recoverHist(const boost::dynamic_bitset<> &history, const FetchStream &entry, int shamt, bool cond_taken)
{
    ...
    if (enableDB) {
        SpecRASTrace rec(When::REDIRECT, RAS_OP::RECOVER, entry.startPC,
            takenSlot.pc, 0, sp, stack[sp].retAddr, stack[sp].ctr);
            specRasTrace->write_record(rec);
    }
    sp = meta_ptr->sp;
    ...
    if (entry.exeTaken) {
        ...
        if (takenSlot.isReturn) {
            if (enableDB) {
                SpecRASTrace rec(When::REDIRECT, RAS_OP::POP,
                    entry.startPC, takenSlot.pc, stack[sp].retAddr, sp,
        ...
        if (takenSlot.isCall) {
            Addr retAddr = takenSlot.pc + takenSlot.size;
            if (enableDB) {
                SpecRASTrace rec(When::REDIRECT, RAS_OP::PUSH, entry.startPC,
                takenSlot.pc, retAddr, sp, stack[sp].retAddr, stack[sp].ctr);
                specRasTrace->write_record(rec);
            }
            push(retAddr, stack, sp);
        }
...
RAS::update(const FetchStream &entry)
{
    ...
    if (entry.exeTaken && (takenSlot.isReturn || takenSlot.isCall)) {
        ...
        auto miss = entry.squashType == SQUASH_CTRL &&
                entry.squashPC == entry.exeBranchInfo.pc;
        if (takenSlot.isCall) {
            Addr retAddr = takenSlot.pc + takenSlot.size;
            if (enableDB) {
                NonSpecRASTrace rec(RAS_OP::PUSH, entry.startPC,
                    takenSlot.pc, retAddr, pred_sp, pred_tos.retAddr,
                    pred_tos.ctr, sp, stack[sp].retAddr, stack[sp].ctr, miss);
        ...
        if (takenSlot.isReturn) {
            if (enableDB) {
                NonSpecRASTrace rec(RAS_OP::POP, entry.startPC,
                    takenSlot.pc, takenSlot.target,
                    pred_sp, pred_tos.retAddr, pred_tos.ctr, sp,
                    stack[sp].retAddr, stack[sp].ctr, miss);
                nonSpecRasTrace->write_record(rec);
            }
            pop(stack, sp);
        }
