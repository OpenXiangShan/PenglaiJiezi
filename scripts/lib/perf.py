...
class gem5PerfCounters(object):
    perf_re = re.compile(r'(([a-zA-Z0-9_]+\.)+(\w|\:)+)\s+(\d+(\.\d+)?)')
    ...
    def __init__(self, filename):
        ...
        with open(filename) as f:
            for line in f:
                ...
                if perf_match:
                    perf_name = str(perf_match.group(1))
                    perf_value = str(perf_match.group(4))
                    perf_name = perf_name.replace("::", ".")
                    all_perf_counters[perf_name] = perf_value

        prefix_length = len(os.path.commonprefix(list(
                            all_perf_counters.keys())))
        updated_perf = dict()
        for key in all_perf_counters:
            updated_perf[key[prefix_length:]] = all_perf_counters[key]

        # print(f"updated_perf: {updated_perf}")
    ...
    def add_manip(self, all_manip):
        ...
        for manip in all_manip:
            if None in map(lambda name: self[name], manip.counters):
                ...
                print(f"Some counters for {manip.name} is not found. \
                      Please check it.")
                continue
            numbers = map(lambda name: float(self[name]), manip.counters)
...
class PerfCounters(object):
    perf_re = re.compile(r'.*\[PERF \]\[time=\s+\d+\] (([a-zA-Z0-9_]+\.)\
                         +[a-zA-Z0-9_]+): ((\w| |\')+),\s+(\d+)$')
    ...
    def __init__(self, filename):
        ...
        with open(filename) as f:
            for line in f:
                ...
                if perf_match:
                    perf_name = ".".join([str(perf_match.group(1)),
                                          str(perf_match.group(3))])
...
def get_rs_manip():
    ...
    alu_bypass_from_mdu = PerfManip(
        ...
        func=lambda fire, b60, b61, b62, b63, b70, b71, b72, b3:
                    (b60 + b61 + b62 + b63 + b70 + b71 + b72 + b3)
        # / fire
    )
    ...
    mdu_bypass_from_mdu = PerfManip(
        ...
        func=lambda fire0, fire1, b40, b41, b50, b51: (b40 + b41 + b50 + b51)
        # / (fire0 + fire1)# if (fire0 + fire1) > 0 else 0
    )
    ...
    load_bypass_from_mdu = PerfManip(
        ...
        func=lambda fire0, fire1, b60, b61, b70, b71: (b60 + b61 + b70 + b71)
        # / (fire0 + fire1)# if (fire0 + fire1) > 0 else 0
    )
...
def get_fu_manip():
    ...
    load_replay_frac = PerfManip(
        ...
        counters=["memScheduler.rs.deq_fire_0",
                  "memScheduler.rs.deq_fire_1",
                  "memScheduler.rs.deq_not_first_issue_0",
                  "memScheduler.rs.deq_not_first_issue_1"],
        func=lambda f0, f1, r0, r1: (r0 + r1) / (f0 + f1) if (f0 + f1) != 0
                    else 0
    )
    # all_manip.append(load_replay_frac)
    store_replay_frac = PerfManip(
        name="global.fu.store_replay_frac",
        counters=["memScheduler.rs_1.deq_fire_0",
                  "memScheduler.rs_1.deq_fire_1",
                  "memScheduler.rs_1.deq_not_first_issue_0",
                  "memScheduler.rs_1.deq_not_first_issue_1"],
        func=lambda f0, f1, r0, r1: (r0 + r1) / (f0 + f1) if (f0 + f1) != 0
                    else 0
    )
...
def get_all_manip():
    ...
    dtlb_miss_rate = PerfManip(
        ...
        counters=[f"memBlock.TLB.first_access0",
                  f"memBlock.TLB.first_miss0",
                  f"memBlock.TLB_1.first_access0",
                  f"memBlock.TLB_1.first_miss0",
                  f"memBlock.TLB_2.first_access0",
                  f"memBlock.TLB_2.first_miss0",
                  f"memBlock.TLB_3.first_access0",
                  f"memBlock.TLB_3.first_miss0"],
        func=lambda req1, miss1, req2, miss2, req3, miss3, req4, miss4:
                (miss1 + miss2 + miss3 + miss4) / (
                req1 + req2 + req3 + req4)
                if ((req1 + req2 + req3 + req4) > 0)
                else 0
    )
    all_manip.append(dtlb_miss_rate)
    dtlb_sa_percent = PerfManip(
        name="global.dtlb_sa_hit_percent",
        counters=[f"memBlock.TLB.tlb_normal_sa.hit",
                  f"memBlock.TLB.tlb_super_fa.hit",
                  f"memBlock.TLB_1.tlb_normal_sa.hit",
                  f"memBlock.TLB_1.tlb_super_fa.hit",
                  f"memBlock.TLB_2.tlb_normal_sa.hit",
                  f"memBlock.TLB_2.tlb_super_fa.hit",
                  f"memBlock.TLB_3.tlb_normal_sa.hit",
                  f"memBlock.TLB_3.tlb_super_fa.hit"],
        func=lambda sa0, fa0, sa1, fa1, sa2, fa2, sa3,
                fa3: (sa0 + sa1 + sa2 + sa3) / (
                sa0 + sa1 + sa2 + sa3 + fa0 + fa1 + fa2 + fa3) if (
                (sa0 + sa1 + sa2 + sa3 + fa0 + fa1 + fa2 + fa3) > 0) else 0
    )
    ...
    ldtlb_sa_percent = PerfManip(
        ...
        counters=[f"memBlock.TLB.tlb_normal_sa.hit",
                  ...
                  f"memBlock.TLB_1.tlb_normal_sa.hit",
                    f"memBlock.TLB_1.tlb_super_fa.hit"],
        func=lambda sa0, fa0, sa1, fa1: (sa0 + sa1) / (sa0 + sa1 + fa0 + fa1)
             if ((sa0 + sa1 + fa0 + fa1) > 0) else 0
    )
    all_manip.append(ldtlb_sa_percent)
    sttlb_miss_rate = PerfManip(
        name="global.sttlb_miss_rate",
        counters=[f"memBlock.TLB_2.first_access0",
                  f"memBlock.TLB_2.first_miss0",
                  f"memBlock.TLB_2.first_access0",
                  f"memBlock.TLB_2.first_miss0"],
        func=lambda req1, miss1, req2, miss2: (miss1 + miss2) / (req1 + req2)
    ...
    sttlb_sa_percent = PerfManip(
        ...
        counters=[f"memBlock.TLB_2.tlb_normal_sa.hit",
                  f"memBlock.TLB_2.tlb_super_fa.hit",
                  f"memBlock.TLB_3.tlb_normal_sa.hit",
                  f"memBlock.TLB_3.tlb_super_fa.hit"],
        func=lambda sa0, fa0, sa1, fa1: (sa0 + sa1) / (sa0 + sa1 + fa0 + fa1)
    ...
    ptw_access_latency = PerfManip(
        ...
        counters=[f"dtlbRepeater.inflight_cycle",
                  f"dtlbRepeater.ptw_req_count"],
    ...
    dcache_load_miss_rate = PerfManip(
        ...
        counters=[f"LoadUnit_0.load_s2.in_fire_first_issue",
                  f"LoadUnit_0.load_s2.dcache_miss_first_issue",
                  f"LoadUnit_1.load_s2.in_fire_first_issue",
                  f"LoadUnit_1.load_s2.dcache_miss_first_issue"],
        func=lambda req1, miss1, req2, miss2: (miss1 + miss2) / (req1 + req2)
    )
    ...
    l3cache_mpki_load = PerfManip(
        ...
        counters=[
            "L3_bank_0_A_channel_AcquireBlock_fire",
            "L3_bank_0_A_channel_Get_fire",
            "L3_bank_1_A_channel_AcquireBlock_fire",
            "L3_bank_1_A_channel_Get_fire",
            "L3_bank_2_A_channel_AcquireBlock_fire",
            "L3_bank_2_A_channel_Get_fire",
            "L3_bank_3_A_channel_AcquireBlock_fire",
            "L3_bank_3_A_channel_Get_fire",
            "commitInstr"
        ],
        func=lambda fire1, fire2, fire3, fire4, fire5,
        fire6, fire7, fire8, instr:
        1000 * (fire1 + fire2 + fire3 + fire4 + fire5 +
                fire6 + fire7 + fire8) / instr
    )
...
def merge_perf_counters(all_perf, verbose=False):
    def extract_numbers(s):
        ...
        pieces = list(map(lambda x: int(x)
                          if x.isdecimal() else x.lower(), pieces))
        return pieces

    all_names = sorted(list(set().union(*list(
        map(lambda s: s.keys(), all_perf)))), key=extract_numbers)
    ...
    suffix_length = get_prefix_length(reversed_names) if \
                    len(filenames) > 1 else 0
    if suffix_length > 0:
    ...
    for name in all_names:
        if verbose:
            ...
            pbar.update(1)
        yield [name] + list(map(lambda perf: perf.get_counter(
            name, strict=True), all_perf))
...
def main(pfiles, output_file, include_names, verbose=False, jobs=1):
    ...
    for i in range(0, jobs):
        p = Process(target=perf_work, args=(all_manip,
                                            work_queue, perf_queue))
...
def find_simulator_err(pfiles):
    if len(pfiles) > 1:
        return sum(map(lambda filename: find_simulator_err(
            [filename]), pfiles), [])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='performance \
                                     counter log parser')
    parser.add_argument('pfiles', metavar='filename', type=str,
                        nargs='*', default=None,
                        help='performance counter log')
    parser.add_argument('--output', '-o', default="stats.csv",
                        help='output file')
    ...
    parser.add_argument('--include', '-I', action='extend',
                        nargs='+', type=str,
                        help="select given counters (using re)")
    parser.add_argument('--jobs', '-j', default=1, type=int,
                        help="processing files in 'j' threads")
